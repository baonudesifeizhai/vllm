...
def fused_all_gather_bmm_fp8(
    ...
    # v1: use semantic-preserving fallback path.
    # Kernel fast-path is intentionally optional while AG+BMM fusion wiring lands.
    if (
...
def fused_bmm_fp8_reduce_scatter(
    ...
    # v1 only handles 2D per-tensor FP8 BMM + reduce_scatter(dim=0).
    if (
        A.ndim != 2
        or B.ndim != 2
        or A.shape[1] != B.shape[0]
