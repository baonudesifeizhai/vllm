...
def _deepseek_v4_mega_moe_pcp_token_shard_op(
    hidden_states: torch.Tensor,
    ...
    self = get_forward_context().no_compile_layers[layer_name]
    self._run_mega_moe_pcp_token_shard(hidden_states, input_ids, out)


def _deepseek_v4_mega_moe_pcp_token_shard_op_fake(
    hidden_states: torch.Tensor,
    input_ids: torch.Tensor | None,
    out: torch.Tensor,
    layer_name: str,
...
direct_register_custom_op(
    op_name="deepseek_v4_mega_moe_pcp_token_shard",
    op_func=_deepseek_v4_mega_moe_pcp_token_shard_op,
    mutates_args=["out"],
    fake_impl=_deepseek_v4_mega_moe_pcp_token_shard_op_fake,
)
...
class DeepseekV4MoE(nn.Module):
    ...
    def forward(
        ...
        if self.use_mega_moe_pcp_token_shard:
            ...
            torch.ops.vllm.deepseek_v4_mega_moe_pcp_token_shard(
                hidden_states,
                input_ids,
                final_hidden_states,
                self.prefix,
