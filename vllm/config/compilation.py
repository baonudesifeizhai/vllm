...
class PassConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.fuse_rope_kvcache and not (
            current_platform.is_rocm() or current_platform.is_cuda_alike()
        ):
            logger.warning_once(
                "KV cache fusion currently only enabled on ROCm or CUDA-like devices. "
                "The fusion will be disabled."
            )
