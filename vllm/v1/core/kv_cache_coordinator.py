...
class HybridKVCacheCoordinator(KVCacheCoordinator):
    ...
    def __init__(
        ...
        self.pcp_world_size = pcp_world_size
        assert all(
            self._effective_block_size(g.kv_cache_spec) % hash_block_size == 0
    ...
    def verify_and_split_kv_cache_groups(self) -> None:
        ...
        # block cache hit yet.
        block_sizes = [
            self._effective_block_size(spec) for spec, _, _ in attention_groups
        ]
    ...
    def find_longest_cache_hit(
        ...
        def _get_block_hashes(kv_cache_spec: KVCacheSpec) -> BlockHashList:
            block_size = self._effective_block_size(kv_cache_spec)
        ...
        while True:
            ...
            for idx, (spec, group_ids, manager_cls) in enumerate(self.attention_groups):
                ...
                if isinstance(spec, FullAttentionSpec) and cached_blocks is not None:
                    ...
                    block_size = self._effective_block_size(spec)
                ...
                if use_eagle:
                    ...
                    block_size = self._effective_block_size(spec)
                    _max_length = min(
                        curr_hit_length + block_size, max_cache_hit_length
                    )
                ...
                _new_hit_length = len(hit_blocks[0]) * self._effective_block_size(spec)
                if use_eagle:
        ...
        if isinstance(spec, FullAttentionSpec):
            num_blocks = hit_length // self._effective_block_size(spec)
